...
function whispli_scripts() {
	...
	if (is_404() ||  is_page_template( 'tmpl-about.php' ) || is_singular( 'product' ) ) {
...
if ( ! function_exists( 'whispli_resize_image' ) ) {
	function whispli_resize_image( $url, $width = null, $height = null, $crop = null, $single = true, $upscale = false ) {
...
if ( ! function_exists( 'whispli_language_selector' ) ) {
	function whispli_language_selector() {
		...
		if ( $main_language ) {
			$language_selector .= '<a class="dropdown-toggle" href="#" data-toggle="dropdown"> <img width="19" src="' . $main_language['country_flag_url'] . '" alt="' . $main_language['language_code'] . '"></a>';
		}
		...
		if ( ! empty( $active_languages ) ) {
			...
			foreach ( $active_languages_ordered as $lang ) {
				$language_selector .= '<li><a href="' . $lang['url'] . '" title="' . ( $settings['icl_lso_display_lang'] ? esc_attr( $lang['translated_name'] ) : esc_attr( $lang['native_name'] ) ) . '">' . ( $settings['icl_lso_display_lang'] ? esc_attr( $lang['translated_name'] ) : esc_attr( $lang['native_name'] ) ) . '<img src="' . $lang['country_flag_url'] . '" alt="' . $lang['language_code'] . '"></a></li>';
			}
